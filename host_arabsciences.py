...
def getinfo():
    ...
    info_['host']= hst
    info_['name']=name
    info_['version']='2.0 26/06/2025'        
    info_['dev']='AbouYacine + Angel_heart'
    info_['cat_id']='21'
...
class TSIPHost(TSCBaseHostClass):
    ...
    def showmenu(self, cItem):          
        self.addDir({'import':cItem['import'],'category' :'host2', 'title':'تصنيفات 1', 'desc':'', 'icon':getinfo()['icon'], 'mode':'20', 'sub_mode': 16})
        self.addDir({'import':cItem['import'],'category' :'host2', 'title':'تصنيفات 2', 'desc':'', 'icon':getinfo()['icon'], 'mode':'20', 'sub_mode': 17})
        self.addDir({'import':cItem['import'],'category' :'host2', 'title':'قنوات البثّ', 'desc':'', 'icon':getinfo()['icon'], 'mode':'20', 'sub_mode': 18})
        self.addDir({'import':cItem['import'],'category' :'host2', 'title':'افلام وثائقية مترجمة', 'desc':'', 'icon':getinfo()['icon'], 'mode':'20', 'sub_mode': 19})
    def showmenu16(self,cItem):#1
    ...
    def showmenu19(self,cItem):#4
        self.addDir({'import':cItem['import'],'category' :'host2', 'url':self.MAIN_URL+'/سلاسل-وثائقيّة-مترجمة/', 'title':'سلاسل وثائقيّة مترجمة', 'desc':'', 'icon':'', 'mode':'20'})
    ...
    def showitms(self,cItem):
        ...
        if sub_mode == 16: return self.showmenu16(cItem)
        elif sub_mode == 17: return self.showmenu17(cItem)
        elif sub_mode == 18: return self.showmenu18(cItem)
        elif sub_mode == 19: return self.showmenu19(cItem)
        URL=cItem['url']
    ...
    def get_links(self, cItem):
        urlTab = []
        url = cItem['url']
        printDBG('get_links() -> url: ' + url)
        sts, data = self.getPage(url)
        if not sts:
            printDBG('get_links() -> Failed to get page content')
            return urlTab
        data = self.decode_unicode_references(data)
        found = False
        video_sources = re.findall(r'<source[^>]+src="([^"]+)"', data, re.I)
        if video_sources:
            found = True
            for video_url in video_sources:
                ...
                label = gethostname(video_url)
                urlTab.append({'name': '|Watch|' + label, 'url': video_url, 'need_resolve': 0})
        iframe_sources = re.findall(r'<iframe[^>]+src="([^"]+)"', data, re.I)
        if iframe_sources:
            found = True
            for iframe_url in iframe_sources:
                ...
                label = gethostname(iframe_url)
                urlTab.append({'name': '|Watch|' + label, 'url': iframe_url, 'need_resolve': 1})
        if not found:
            printDBG('get_links() -> No valid video or iframe source found.')
        return urlTab
    ...
    def getVideos(self, videoUrl):
        urlTab = []
        printDBG('getVideos() -> videoUrl: ' + videoUrl)
        sts, data = self.getPage(videoUrl)
        if not sts:
            printDBG('getVideos() -> Failed to retrieve page')
            return urlTab
        data = self.decode_unicode_references(data)
        iframes = re.findall(r'<iframe[^>]+src="([^"]+)"', data, re.I)
        printDBG('getVideos() -> Found iframes: {}'.format(str(iframes)))
        if iframes:
            for iframe_url in iframes:
                ...
                if iframe_url.startswith('//'):
                    iframe_url = 'https:' + iframe_url
                label = gethostname(iframe_url)
                printDBG(f'getVideos() -> iframe_url: {iframe_url}')
                urlTab.append({'name': '|Watch|' + label, 'url': iframe_url, 'need_resolve': 1})
        else:
            printDBG('getVideos() -> No iframe links found.')
        return urlTab
